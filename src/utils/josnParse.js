...
let restaurants = document.querySelectorAll(".webfound-refresh");

for (let i = 3; i < restaurants.length; i++) {
  let element = restaurants[i].children[0]?.children[0]?.children || null;

  if (element) {
    let arr = Array.from(element);

    let c = arr[1]?.children[1]?.children[0]?.textContent || "b";
    const obj = {
      no: i - 2,
      id: Date.now() + i,
      name:
        arr[1]?.children[0]?.children[0]?.textContent ||
        arr[1]?.children[0]?.children[1]?.textContent ||
        null,
      top: arr[1]?.children[0]?.children[1]?.textContent !== null,
      image:
        arr[0]?.children[0]?.children[0]?.src ||
        arr[0]?.children[1]?.children[0]?.src ||
        imageGenerator(c),
      ratings:
        arr[1]?.children[0]?.children[1]?.children[1]?.textContent ||
        ratingsGenerator(c),
      reviews:
        arr[1]?.children[0]?.children[1]?.children[2]?.textContent ||
        reviewsGenerator(c),
      offer:
        arr[0]?.children[1]?.children[0]?.children[1]?.textContent ||
        offerGenerator(c),
      cuisine: arr[1]?.children[1]?.children[0]?.textContent || null,
    };
...
let jsonResult = JSON.stringify(newarray);
...
// if(arr[1]?.children[0]?.children[1]?.textContent){
//     const obj = {

//       name:arr[1]?.children[0]?.children[1]?.textContent ||
//         null,
//       cuisine: arr[1]?.children[1]?.children[0]?.textContent || null,
//     };
//     newarray.push(obj);
